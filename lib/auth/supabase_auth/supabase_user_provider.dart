...
class VisualRealitySupabaseUser extends BaseAuthUser {
  VisualRealitySupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> visualRealitySupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = VisualRealitySupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
